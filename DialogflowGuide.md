...
&emsp;&emsp;<a href="#7">1.2.4 何为模板代理</a>  <br>
...
模板代理是我们根据公司业务抽象而来，而谷歌本身没有模板代理的概念。模板代理产生的原因是为了做到商家的数据分割、场景的业务意图统一。拿餐饮场景来举例，开发人员维护一个模板代理，我们希望餐饮场景下的所有项目都具有同样的语义功能，每次新增一个餐饮项目，一个谷歌空代理会被创建，该餐饮项目的代理会把模板代理复制一份，然后直接将其覆盖到该商家代理，这样此商家代理就具有了模板代理的所有功能。 值得注意的是，不同的商家配置了不同的菜名、口味等。如何保证商家的数据隔离呢？ 拉取模板代理后，我们会判断哪些是动态实体，然后将该商家的动态实体更新到与之对应的谷歌代理。
...
#### <a name="9">1.2.4.2  模板代理的版本控制</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>

上面提到一个谷歌代理具有流级别的版本控制功能，我们可以为每个流保存一个版本，然后我们自定义环境来指向不同版本的流。 谷歌本身没有代理版本的概念，但是通过新建环境来指向流版做到了代理版本的控制。 现在我们的做法是，在模板代理流设置3个环境，即研发、生产、测试。 每个环境的代理有唯一个id，我们可以通过环境id来访问不同版本的代理。如下图：<br>
...
#### <a name="10">1.2.4.3 父模板代理与“复制品”的关系</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>

##### 什么是父模板代理？
父模板代理是bot开发人员的主要操作对象，是根据公司业务取名而来，谷歌本身没有父模板代理或模板代理概念。在一个公司场景、一个语言下，我们只设置一个父模板代理，其他地区的同样场景和语言的代理是从父模板处复制而来。我们约定将香港的模板代理作为父模板代理，而全球其他地区的同场景同语言的模板代理只从父模板代理处复制。比如“香港（英文）餐饮代理”是父模板，“美国（英文）餐饮代理"就是香港英文餐饮代理的复制品。 <br>
##### 父模板代理的环境
当前我们在父模板代理中自定义研发、生产、测试环境，但其“复制品”代理无环境，复制品从父模板代理复制后默认保存在草稿代理。简单来说，“复制品”代理存在的意义只是为了地区访问速度而设置。为每个复制品代理设置环境并无意义。商家项目的代理会遵从地理位置就近的原则，从最近的模板代理的草稿拉去数据。 

因此在每次语义版本迭代的时候，通过研发和测试调试代理无误后，我们将生产环境指向该代理。然后将该代理导出，然后手动导入到“复制品”代理。<br>


#### <a name="11">1.2.4.4 谷歌项目和代理命名规则</a><a style="float:right;text-decoration:none;" href="#index">[Top]</a>
...
1.谷歌地图项目：GoogleMap <br>
<br>
2.父模板代理的谷歌项目： <br>
当前所有父模板存放于“HkRestaurantEnglishTemplate”，后续会将名字规范化。<br>
<br>
3.“复制品”代理的谷歌项目：<br>
复制品代理项目以“地区+语言+Template方式命名”，如“UsaRestaurantEnglishTemplate”，后续会将名字规范化。<br>
<br>
4.生产环境谷歌项目：<br>
生产环境的谷歌项目用于存放商家的代理，项目名以“地区+公司场景+语言+Project”结尾，例子如下：<br>
UsaRestaurantEnglishProject 美国餐饮英语生产环境项目<br>
HkRestaurantEnglishProject 香港餐饮英语生产环境项目<br>
HkOfficeEnglishProject  美国写字楼英语生产环境项目<br>
UsaOfficeEnglishProject 美国写字楼英语生产环境项目<br>
<br>
5.语音识别、语音播报服务的项目：<br>
项目名称固定为 “STTproject” <br>
<br>
6.测试环境谷歌项目：<br>
项目名称固定为“TestProject ”，用于存放所有场景下测试环境的商家代理。<br>
<br>
7.研发环境谷歌项目： <br>
项目名称固定为“DevelopmentProject”，用于存放所有场景下研发环境的商家代理。<br>
<br>
#### 最终做法

 - 当前我们为一个地区下一个场景的“复制品”代理新建了一个谷歌项目，如下图的 “UsaRestaurantEnglishTemplate” ，这样做不便于管理。后期我们会只以“复制品”代理的地区和语言来划分，比如
项目名为"UsaTemplate"只存放美国地区的各大场景的模板代理复制品，


![image](https://user-images.githubusercontent.com/30898964/168939207-85bb13f7-4338-4e97-abfb-ea69b828e7e1.png)


![image](https://user-images.githubusercontent.com/30898964/168939400-5f4603df-7e57-4fb9-85ed-ed4485765774.png)
...
我们设立研发环境的谷歌项目（DevelopmentProject）和测试环境的谷歌项目（TestProject）、生产环境的谷歌项目。这三个项目从哪里复制模板代理数据？
在上面提到了我们只在父模板代理里自定义研发、测试、生产环境，谷歌研发项目中所有的代理只从父模板代理的研发环境复制。同理，谷歌测试项目中所有代理只从父模板代理的测试环境复制。而生产环境的谷歌代理处理方式不同，我们需要把父模板的生产环境的代理导入到各大区的“复制品”模板代理。语义服务数据库已经存放好表，该表指明了生产环境的某商家项目该从哪个模板代理去复制（见下图）。<br>
从模板代理处拉取代理数据后，该商家代理会清空所有数据并上传模板代理(包括意图、实体等)。然后，该商家代理会更新自己的动态实体，并完成自动训练。<br>
<br>
...
我们有6大场景，每个场景都有对应的模板代理、测试环境谷歌项目、生产环境谷歌项目。具体关系如下图所示。
